...
export function SocialSanctuary({ isDarkMode, isGirlyMode, currentUser, userProfile }: SocialSanctuaryProps) {
  ...
  const filteredPeers = peers.filter(peer => 
    peer.name.toLowerCase().includes(peerSearchQuery.toLowerCase()) ||
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="text-[10px] font-black uppercase tracking-tight truncate">{otherName}</p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {filteredPeers.map(peer => (
                  <div
                    key={peer.uid}
                    className={cn(
                      "p-5 rounded-3xl border flex flex-col justify-between gap-4 transition-all duration-300 relative overflow-hidden group",
                  ...
                  >
                    <div className="space-y-2 relative z-10 text-xs">
                      <div className="flex items-center gap-2.5">
                        <img 
                          src={peer.avatarUrl || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=200'}
                          className="w-10 h-10 rounded-full object-cover shrink-0 border border-zinc-500/10"
                          alt="peer"
                          referrerPolicy="no-referrer"
                        />
                        <div className="min-w-0">
                          <h4 className="font-black uppercase tracking-tight truncate">{peer.name}</h4>
                          <span className="text-[8px] px-1.5 py-0.5 rounded bg-emerald-500/15 text-emerald-500 font-extrabold uppercase tracking-widest text-center border border-emerald-500/10">
                            Seeker
                          </span>
                        </div>
                      </div>
                      ...
                      <p className={cn(
                        "text-[11px] leading-relaxed line-clamp-3 font-normal min-h-[42px]",
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        onClick={() => handleStartDM(peer)}
                        className="flex-1 py-2 rounded-xl bg-emerald-500 text-zinc-950 font-black italic uppercase text-[9px] tracking-wider active:scale-95 transition-transform flex items-center justify-center gap-1"
                      >
