...
function ArticleCard({ 
  ...
  const [isExpanded, setIsExpanded] = useState(() => {
    if (typeof window !== 'undefined') {
      ...
      const targetId = params.get('id') || params.get('articleId');
      if (targetId === article.id) return true;
    }
    return false;
  });
  ...
  const isLongContent = (article.content || '').length > 350;
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-4 w-full" id={`article-header-text-${article.id}`}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-x-2 gap-y-1 mt-1.5">
            <p className={cn("text-[10px] uppercase font-bold tracking-widest", isDarkMode ? "text-zinc-400" : "text-zinc-500")}>
              {format(new Date(article.date), 'MMM d, yyyy • HH:mm')}
            </p>
            <span className={cn("text-[10px]", isDarkMode ? "text-zinc-600" : "text-zinc-300")}>•</span>
            <span className={cn(
      ...
      {/* Video Player (placed right after header for maximum video visibility while scrolling) */}
      {article.url && (
        <div className="relative rounded-[2rem] overflow-hidden border border-zinc-500/10 bg-black aspect-video my-2 shadow-xl">
          <video 
            ref={videoRef}
      ...
      {/* Article Content with Read More / Collapsible Teaser */}
      <div className="relative">
        <div className={cn(
          "markdown-body text-[16px] sm:text-[17px] md:text-[18px] leading-relaxed sm:leading-loose tracking-wide max-w-none font-medium transition-all duration-300",
          isDarkMode ? "text-zinc-100" : "text-zinc-850",
          (!isExpanded && isLongContent) && "max-h-[145px] overflow-hidden select-none"
        )}>
        <ReactMarkdown
          components={{
            strong: ({ children }) => {
        ...
        {/* Delicate Bottom Gradient Fade Overlay when collapsed */}
        {!isExpanded && isLongContent && (
          <div 
            onClick={() => setIsExpanded(true)}
            className={cn(
              "absolute inset-x-0 bottom-0 h-28 bg-gradient-to-t pointer-events-auto cursor-pointer flex items-end justify-center pb-2 transition-opacity group/fade",
              isDarkMode 
                ? "from-zinc-900 via-zinc-900/80 to-transparent" 
                : "from-white via-white/85 to-transparent"
            )}
            title="Click to expand full article"
          />
        )}
      </div>
        ...
        <div className="flex justify-center pt-2 pb-1">
          {!isExpanded ? (
            <button
              ...
              className={cn(
                "group flex items-center gap-2 px-5 py-2.5 rounded-full border text-xs font-black uppercase tracking-wider transition-all duration-200 active:scale-95 shadow-sm hover:shadow",
                isDarkMode 
                  ? "bg-zinc-850/90 border-zinc-750 hover:border-emerald-500/50 hover:bg-zinc-800 text-emerald-400" 
                  : "bg-zinc-50 border-zinc-250 hover:border-emerald-300 hover:bg-emerald-50/50 text-emerald-600"
              )}
            >
              <BookOpen className="w-3.5 h-3.5 text-emerald-500 group-hover:scale-110 transition-transform" />
              <span>Read Full Article • {readingTimeMinutes} min read</span>
            ...
            <button
              ...
              onClick={() => {
                setIsExpanded(false);
                const cardEl = document.getElementById(`article-${article.id}`);
                if (cardEl) {
                  cardEl.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
                }
              }}
              id={`btn-show-less-${article.id}`}
              className={cn(
                "group flex items-center gap-2 px-4 py-2 rounded-full border text-[11px] font-bold uppercase tracking-wider transition-all duration-200 active:scale-95",
                isDarkMode 
                  ? "bg-zinc-850/60 border-zinc-800 hover:bg-zinc-800 text-zinc-400 hover:text-zinc-200" 
                  : "bg-zinc-100/80 border-zinc-200 hover:bg-zinc-200/70 text-zinc-600 hover:text-zinc-900"
              )}
            >
              <span>Show Less</span>
...
function AppContent() {
  ...
  const handleAddArticle = async () => {
    ...
    if (charCount > 80000) {
      alert("Article is too long! (Max 80,000 characters)");
      return;
    }
    if (wordCount > 13000) {
      alert("Article has too many words! (Max 13,000 words)");
      return;
    }
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <div className="flex gap-3 text-[10px] font-bold">
                        <span className={cn(
                          articleContent.length > 80000 ? "text-red-500" : "text-zinc-500"
                        )}>
                          {articleContent.length.toLocaleString()} / 80,000 chars
                        </span>
                        <span className={cn(
                          articleContent.trim().split(/\s+/).filter(Boolean).length > 13000 ? "text-red-500" : "text-zinc-500"
                        )}>
                          {articleContent.trim().split(/\s+/).filter(Boolean).length.toLocaleString()} / 13,000 words
                        </span>
                      </div>
                    </div>
                    <textarea 
